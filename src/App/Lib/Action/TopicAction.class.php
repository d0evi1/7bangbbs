...
class TopicAction extends Action {
	...
	public function article() 
	{
		...
		// step 1: 更新文章计数.
		$model = new Model();
		$model->execute("update bbs_articles set views=views+1 where id=".$cur_art_id);
		
		// step 2: 获取文章信息
		$Articles = new Model();
		$art = $Articles->table('bbs_articles articles, bbs_users users')
		...
		// step 3: 获取对应的评论信息
		$comments = $model->table('bbs_comments comments, bbs_users users')
		...
		// step 5: 相关帖子
		$cur_cid = $article['cid'];
		$t_relative_articles = $model->query("select * from bbs_articles where cid =".$cur_cid." and id!=".$cur_art_id." order by rand() limit 2");
	...
	public function new_topic() 
	{
		...
		//  类别选取.
		$Categories = M('bbs_categories');
		$cats = $Categories->select();
		$this->assign('t_categories', $cats);
	...
	public function edit() 
	{
		...
		if($this->isPost())
		{
			...
			if($ret)
			{
				header("location: /topic/article/".$art_id);
			}
			else
			{
				$this->error("更新文章失败!");
